...
import { parseSellerFile, validateFile, formatFileSize } from "@/lib/utils/csv-parser"
import { ERROR_MESSAGES } from "@/lib/constants"

interface SellerFileUploaderProps {
  onFileUpload: (data: SellerFileData) => void
}

const SellerFileUploader = ({ onFileUpload }: SellerFileUploaderProps) => {
  ...
  const handleFileSelect = async (files: FileList) => {
    if (files.length > 0) {
      ...
      if (!validation.valid) {
        showToast(validation.error!, "warning", "bottom-right")
        return
      }
      ...
      try {
        const data = await parseSellerFile(file)

        setUploadedFile(file)
        onFileUpload(data)
      } catch (error) {
        showToast(error instanceof Error ? error.message : ERROR_MESSAGES.PARSE_ERROR, "error", "bottom-right")
      } finally {
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-xl font-bold mb-3 text-gray-900">
                {isProcessing ? "Processing file..." : "Upload Seller Product File"}
              </h3>

              <p className="mb-6 text-lg text-gray-600">
                {isProcessing
                  ? "Parsing product data and extracting columns..."
                  : "Drag and drop your product CSV here, or click to browse"}
              </p>
              ...
              <div className="mt-6 text-sm text-gray-500 bg-gray-100 p-4 rounded-lg max-w-2xl mx-auto">
                ...
                <p className="text-left">
                  • <strong>Headers</strong>: SKU, Name, BrandName, Gender, Category, Color, Size, MRP, Price, Material,
                  Image1, Image2, Quantity, Description
                  <br />• <strong>First row</strong>: Column headers
                  <br />• <strong>Data rows</strong>: One product per row
                  <br />• <strong>Format</strong>: Standard CSV with comma-separated values
                </p>
              </div>
          ...
          <div className="p-6 border rounded-2xl bg-white shadow-md flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">{formatFileSize(uploadedFile.size)} • CSV file</p>
              </div>
            </div>
...
export default SellerFileUploader
